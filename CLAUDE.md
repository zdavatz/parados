...
**Localization by duplication:** Language variants are separate files (e.g., `kangaroo.html` for German, `kangaroo_jp.html` for Japanese, `kangaroo_cn.html` for Chinese). When modifying game logic, changes must be propagated to all language variants.

**Games:**
- `kangaroo.html` (+ `_jp`, `_cn`) — DUK (Das Ungeduldige Känguru): grid-based puzzle with kangaroo jumping over colored dishes. Supports LEVEL, RANDOM, and EDITOR modes. Has a `LEVEL_DATABASE` array for predefined puzzles.
