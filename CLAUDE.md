...
- **Undo:** `↶ Undo move` button in `#game-actions`. Every `saveSnapshot(desc)` captures full state (`cells`, `bag` contents, `gameState`, `scores`, `playerFlags`, `collectors`, `collectedDiscs`, `relinquished`/`relinquishCount`, `draftChosen`, `pendingExtraTurn`, `selectedIdx`, `sacrificeRange`, `sacrificingPlayer`, `isFirstMove`). `undoMove()` pops the last snapshot and restores live state from the new head, hides draft/sacrifice panels, exits review mode. Disabled when `moveHistory.length <= 1` or `gameState === 'END'`. `applyPositionData()` resets `moveHistory` + all per-game flags so test-playing a custom from the editor after a curated game starts fresh.
- **Custom-position editor (local only):** `mlOpenEditor(null)` opens the 12×12 painter. Brush tokens — `CB`/`CW` for collectors (exactly one of each enforced on paint), `R1..R6`/`B1..B6`/`Y1..Y6` for primary discs, `G`/`O`/`P` for secondaries; paint keyed by `y*12+x`. Save → `localStorage["ml_custom_positions"]` as `{customId, name, b:[x,y], w:[x,y], d:[[x,y,color,num],...]}`, the same shape as `CURATED_POSITIONS` entries so the load path is the shared `applyPositionData(pos)` extracted from `loadCuratedPosition`. CSV export emits the same long format as `makalaina_starting_positions.csv`. Share via `navigator.share({files:[csv]})` with no `text:` (Android WhatsApp file-drop bug); fallback = download only — never `wa.me` (`ERR_UNKNOWN_URL_SCHEME`).
