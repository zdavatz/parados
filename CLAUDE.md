...
- `makalaina.html` / `makalaina_remote.html` — MAKA LAINA: 2-player disc collection strategy on a 12x12 grid. 54 discs total (36 primary in red/blue/yellow with numbers 1-6, 18 secondary in green/orange/purple). 6 hidden, 24 start on board, 24 drafted in. Draft system: after each collection, 3 discs drawn from bag; player picks 1 to place on the board (other 2 return to bag), placing it in their line of sight (LOS). Drafted discs in LOS cannot be collected immediately (collection requires discs outside LOS). LOS highlighting is off by default, toggled via "Show Line of Sight" button — when on, highlights LOS cells during both regular play and draft placement. For the first 24 turns disc count stays at 24 on board (each collected disc replaced by 1 from bag); after bag empties, count decreases. Sacrifice mechanic: give up a collected primary disc (score reduced), move collector freely up to that disc's value in squares (Chebyshev distance). Number 1 = extra turn, Number 6 = future discs of that color worthless. Game continues until all discs collected. Plan from the get go, stay flexible. Remote version (`makalaina_remote.html`) has full feature parity with local version: uses PeerJS for networking, with message types for clicks, sacrifice selections (`sacrifice_pick`/`sacrifice_cancel`), relinquish, and full state sync (including bag and collected discs).
