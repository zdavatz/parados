...
- `makalaina.html` / `makalaina_remote.html` — MAKA LAINA: 2-player disc collection strategy on a 12x12 grid. 54 discs total (36 primary in red/blue/yellow with numbers 1-6, 18 secondary in green/orange/purple). 6 hidden, 24 start on board, 24 drafted in. Draft cycle (3-phase): after Player A collects, 3 discs drawn from bag; A picks 1 and places it in their line of sight (LOS); Player B collects, picks 1 of remaining 2 and places it in their LOS; A takes the last disc and places it in their LOS. Drafted discs must be placed in the placing player's LOS so they cannot be collected immediately (collection requires discs to be outside LOS). Starting player alternates each cycle. Repeats 8 times (24 discs). For the first 24 turns disc count stays at 48 (each collected disc is replaced); after that count decreases. Extra turns from "1" are disabled during active draft cycles (only apply after bag is empty). Sacrifice mechanic: give up a collected primary disc (score reduced), move collector freely up to that disc's value in squares (Chebyshev distance). Number 1 = extra turn (post-draft only), Number 6 = future discs of that color worthless. Game continues until all discs collected. Plan from the get go, stay flexible. Remote version (`makalaina_remote.html`) has full feature parity with local version: uses PeerJS for networking, with message types for clicks, draft picks (`draft_pick`), sacrifice selections (`sacrifice_pick`/`sacrifice_cancel`), relinquish, and full state sync (including bag, draft cycle, and collected discs).
